...
**The rest of the surface area.** Built on the same two core features, the app also includes: full **shift CRUD** (edit/delete with a 5-second undo) and **backdated logging** (`PATCH` recomputes `week_label` server-side — see §4); **CSV statement import** (`POST /shifts/bulk`, batched write, max 200 rows) and a **raw CSV export**; a filtered **dashboard** with this-week/month/all-time scopes, period-navigation chevrons with honest past-period labels, per-platform charts, and a month-end **projection** with an optional *zakat pendapatan* (2.5%) toggle; **expense tracking** (fuel/data/maintenance/other) surfaced as expense-adjusted net income; **rule-based insight cards** (weekly-goal pacing, week-over-week delta, best platform, strongest day, i-Saraan nudge) computed client-side from already-fetched data; a **weekly goal** and **weekly digest**; the **Protection tab** (SOCSO contribution ledger, i-Saraan Plus card, work-accident/PERKESO claim guidance, rights explainers, PERKESO/KWSP links); and experience touches — **bilingual EN/BM**, an **installable PWA** with offline-tolerant runtime caching, an optional **evening reminder** notification, and subtle **reward animations** on logging. Insight and compliance values are all pure functions of fetched data, so they update instantly when a shift is logged without an extra round-trip.
